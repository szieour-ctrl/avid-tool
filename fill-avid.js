...
// Accepts: multipart POST with fields { transcript, pdf (base64) }
//   OR JSON POST with { transcript, pdfBase64 }
// Returns: filled PDF as base64 JSON { pdfBase64, filename }
//
// Two-step pipeline:
//   1. Call Claude to extract inspection JSON from transcript
//   2. Fill the PDF with pdf-lib using extracted JSON
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!transcript) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: "transcript is required" }) };
    }

    // Step 1: Extract inspection data from transcript via Claude
    const data = await extractFromTranscript(transcript);

    // Step 2: Fill PDF if provided, otherwise just return JSON
    if (pdfBase64) {
      ...
      return {
        ...
        body: JSON.stringify({
          pdfBase64: filledBase64,
          filename: `AVID_${address}.pdf`,
          data,
        }),
      };
    }

    // No PDF — return JSON only (for artifact use)
    return {
      statusCode: 200,
      headers: { ...headers, "Content-Type": "application/json" },
